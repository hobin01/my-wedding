// ─────────────────────────────────────────────────────────────────────────────
// 모바일 청첩장 — 사용자 설정 파일 (Single Source of Truth)
//
// 본인만의 청첩장을 만들려면 이 파일만 수정하세요.
// 이미지는 assets/ 와 assets/gallery/ 디렉토리에 그대로 넣으면 됩니다.
// 값 변경 후 브라우저 새로고침만 하면 반영됩니다.
// ─────────────────────────────────────────────────────────────────────────────
window.CONFIG = {
  // ── 메타 정보 ──────────────────────────────────────────────────────────────
  meta: {
    title: '이도현 · 한지우의 결혼식에 초대합니다',
    description: '2026년 11월 7일 토요일 오후 1시 30분 · 그랜드 인터컨티넨탈 서울',
    // 배포될 절대 URL (끝에 / 포함). 카카오톡 공유 이미지 생성 시 base 로 사용.
    siteUrl: 'https://hb4258.github.io/',
    // assets/ 기준 상대 경로. og.jpg 권장 사양: 1200×630, JPG, ≤300KB
    ogImage: 'og.jpg',
    // assets/ 기준 상대 경로. favicon.png 권장 사양: 180×180, PNG
    favicon: 'favicon.png',
    // Cover 섹션 상단에 표시되는 대표 사진. assets/ 기준 파일명.
    // null 이면 기존의 순수 텍스트 Cover 레이아웃이 유지됩니다.
    // 권장 사양: 세로 긴 비율 (예: 960×1200), JPG, ≤400KB.
    heroImage: 'hero.jpg',
    // 카카오 JavaScript 키. 빈 문자열이거나 "DUMMY"/"YOUR_" 접두어가 있으면 비활성.
    // 하나의 JS 키로 "카카오톡 공유" + "카카오맵 JS SDK" 둘 다 사용 가능합니다.
    kakaoAppKey: '0ea5558172608f8c2db792e5dfe27dc3',
  },

  // ── 인용구 (Cover 와 초대의 글 사이에 표시) ────────────────────────────────
  // text 가 빈 문자열이면 인용구 블록 자체가 숨겨집니다.
  quote: {
    text: '사랑은 서로를 마주 보는 것이 아니라\n함께 같은 방향을 바라보는 것이다.',
    author: '생텍쥐페리, 《인간의 대지》',
  },

  // ── 신랑 ──────────────────────────────────────────────────────────────────
  groom: {
    name: '이도현',
    nameEn: { first: 'Do Hyun', full: 'LEE DO HYUN' },
    // 부모 자녀 호칭 (예: 장남, 차남, 장녀, 차녀 등)
    roleKo: '장남',
    parents: {
      father: '이정우',
      mother: '박수진',
      fatherRole: '아버지',
      motherRole: '어머니',
      // 빈 문자열이면 해당 부모 연락처 버튼이 표시되지 않습니다.
      fatherPhone: '010-1111-2222',
      motherPhone: '010-3333-4444',
    },
    phone: '010-1234-5678',
    // assets/ 기준 파일명. 권장 사양: 480×600, JPG, ≤200KB
    // 파일이 없으면 그라디언트 플레이스홀더로 자동 대체됩니다.
    photo: 'groom.jpg',
  },

  // ── 신부 ──────────────────────────────────────────────────────────────────
  bride: {
    name: '한지우',
    nameEn: { first: 'Ji Woo', full: 'HAN JI WOO' },
    roleKo: '차녀',
    parents: {
      father: '한성호',
      mother: '김은영',
      fatherRole: '아버지',
      motherRole: '어머니',
      fatherPhone: '010-5555-6666',
      motherPhone: '010-7777-8888',
    },
    phone: '010-9876-5432',
    photo: 'bride.jpg',
  },

  // ── 일시 ──────────────────────────────────────────────────────────────────
  // ISO 8601 형식 하나로 통일. 캘린더/D-Day/시간 표시 전부 여기서 파생됩니다.
  date: {
    datetime: '2026-11-07T13:30:00+09:00',
  },

  // ── 장소 ──────────────────────────────────────────────────────────────────
  // lat/lng 은 카카오맵 인터랙티브 지도 중심 + 마커 좌표로 사용됩니다.
  venue: {
    name: '그랜드 인터컨티넨탈 서울',
    hall: '그랜드볼룸',
    floor: '지하 2층',
    address: '서울특별시 강남구 테헤란로 521',
    addressDetail: '삼성역 5번 출구 도보 3분',
    phone: '02-555-5656',
    lat: 37.5087,
    lng: 127.0632,
  },

  // ── 초대의 글 ─────────────────────────────────────────────────────────────
  // \n 줄바꿈 그대로 표시됩니다.
  invitation: `서로가 마주 보며 다져온 사랑을
이제 함께 한 곳을 바라보며
걸어갈 수 있는 큰 울타리로 키우고자 합니다.

오랜 기다림 끝에 맺어지는 귀한 걸음에
부디 오셔서 축복해 주시면 감사하겠습니다.`,

  // ── 오시는 길 — 교통편 안내 ───────────────────────────────────────────────
  // icon 은 원 안에 표시될 한 글자 (M/B/P 추천) 또는 이모지.
  transport: [
    { label: '지하철', icon: 'M', detail: '2호선 삼성역 5번 출구 도보 3분' },
    { label: '버스', icon: 'B', detail: '간선 146, 360 / 지선 2413 · 코엑스 하차' },
    { label: '자가용', icon: 'P', detail: '호텔 발레파킹 가능 (연회 당일 무료)' },
  ],

  // ── 하객 안내 ─────────────────────────────────────────────────────────────
  // 식사/주차/화환/드레스 코드 등 예식 당일 실용 정보. 자유롭게 추가·제거하세요.
  // 빈 배열이면 "안내사항" 섹션 자체가 표시되지 않습니다.
  notices: [
    { icon: '🍽', title: '식사 안내',   detail: '예식 종료 후 같은 층 연회장에서 한식 뷔페가 제공됩니다.' },
    { icon: '🅿', title: '주차 안내',   detail: '호텔 지하 주차장 3시간 무료. 입구에서 주차권을 받아주세요.' },
    { icon: '💐', title: '화환 안내',   detail: '양가 합의 하에 화환은 정중히 사양합니다.' },
    { icon: '👗', title: '드레스 코드', detail: '별도 드레스 코드는 없으니 편한 복장으로 오세요.' },
  ],

  // ── 계좌 안내 ─────────────────────────────────────────────────────────────
  // name 에 "(父)", "(母)" 등 보조 라벨을 붙여도 됩니다. 복사 시 하이픈은 자동 제거.
  accounts: {
    // 섹션 상단의 안내 멘트. \n 줄바꿈 지원.
    note: '참석이 어려우신 분들을 위해\n계좌번호를 안내드립니다',
    groom: [
      { name: '이도현', bank: '국민은행', number: '123-456-789012' },
      { name: '이정우 (父)', bank: '신한은행', number: '110-234-567890' },
      { name: '박수진 (母)', bank: '하나은행', number: '234-567890-123' },
    ],
    bride: [
      { name: '한지우', bank: '우리은행', number: '1002-345-678901' },
      { name: '한성호 (父)', bank: '농협', number: '301-2345-6789-01' },
      { name: '김은영 (母)', bank: '카카오뱅크', number: '3333-12-3456789' },
    ],
  },

  // ── 갤러리 ────────────────────────────────────────────────────────────────
  // files 를 null 로 두면 assets/gallery/ 아래 NN.{jpg,jpeg,png,webp} 파일을
  // 01 부터 순서대로 자동 탐색합니다 (빈 번호가 나오면 중단).
  // 직접 지정하려면 files: ['a.jpg', 'b.png', ...] 형태로 배열을 넣으세요.
  gallery: {
    files: null,
    maxScan: 30,
  },
};
